...
public isolated client class Client {
    ...
    # + return - No content 
    resource isolated function delete [string contactId](map<string|string[]> headers = {}) returns http:Response|error {
    ...
    resource isolated function get .(map<string|string[]> headers = {}, *ReadPageOfContactsQueries queries) returns CollectionResponseSimplePublicObjectWithAssociationsForwardPaging|error {
    ...
    # + queries - Queries to be sent with the request 
    # + return - successful operation 
    resource isolated function get [string contactId](map<string|string[]> headers = {}, *ReadContactByIdQueries queries) returns SimplePublicObjectWithAssociations|error {
        string resourcePath = string `/${getEncodedUri(contactId)}`;
        map<anydata> headerValues = {...headers};
        if self.apiKeyConfig is ApiKeysConfig {
            headerValues["private-app"] = self.apiKeyConfig?.private\-app;
    ...
    # + queries - Queries to be sent with the request 
    # + return - successful operation 
    resource isolated function post batch/read(BatchReadInputSimplePublicObjectId payload, map<string|string[]> headers = {}, *ReadBatchOfContactsByInternalIdOrUniquePropertyValuesQueries queries) returns BatchResponseSimplePublicObject|BatchResponseSimplePublicObjectWithErrors|error {
    ...
    resource isolated function post gdpr\-delete(PublicGdprDeleteInput payload, map<string|string[]> headers = {}) returns http:Response|error {
